...
| **Relatórios** | PDF individual — resumo executivo, checklist numa página, notas em lista, fecho (próximas → declaração → assinaturas) | ✅ |
| **Email** | Corpo HTML UTF-8 (sem entidades visíveis) + PDF FPDF alinhado ao browser (`gerarPdfCompacto`) | ✅ |
...
# Testes unitários (Node test runner, sem browser)
npm run test:unit
...
| Testes | Playwright E2E — ver `docs/TESTES-E2E.md` (452 listados em 19 ficheiros); `npm run test:e2e` / `npm run test:e2e:last-failed` · Unitários: `npm run test:unit` (114 testes em `tests/unit/`) |
