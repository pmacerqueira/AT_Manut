...
| Relatório individual PDF | `gerarPdfRelatorio.js` (`gerarPdfCompacto`), `relatorioPdfResumo.js`, `relatorioManutencaoPayload.js`, `execWizardHelpers.js`, `relatorioBaseStyles.js` |
| Relatório de frota | `gerarRelatorioFrotaHtml.js`, `gerarRelatorioFrota.js`, `frotaReportHelpers.js`, `Clientes.jsx` |
| Relatório de reparação (email/PDF) | `servidor-cpanel/send-email.php` (FPDF + HTML), `gerarPdfRelatorio.js` |
| Envio de email (manutenção, lembretes) | `emailService.js` (`resumo_executivo_json`, `quick_notes_json`), `EnviarEmailModal.jsx`, `servidor-cpanel/send-email.php` (`atm_html_esc`, `atm_linhas_notas_relatorio`) |
