...
.rpt-assinatura-img img {
  ...
  margin-top: 6px; background: var(--branco); display: block;
}
.rpt-assinatura-placeholder {
  height: 60px; border-bottom: 1.5px dashed var(--borda);
...
<header class="rpt-header" style="display:flex;align-items:flex-start;justify-content:space-between;gap:14px;padding-bottom:10px;border-bottom:3px solid ${e.azulNavel}">
  ...
  <div class="rpt-empresa" style="text-align:left;font-size:${t.pequeno};line-height:1.55;color:${e.texto}">
    <strong style="font-size:${t.corpo};color:${e.azulNavel}">${a(n.nome)}</strong><br>
    ${a(n.divisaoComercial)}<br>
    ${a(n.sede)}<br>
    ${a(n.telefones)} &nbsp;|&nbsp; <a href="https://${n.web}" style="color:${e.azulNavel};text-decoration:none;font-weight:600">${n.web}</a><br>
    ${a(n.pais)}
  </div>
...
<div class="rpt-acento" style="height:2px;background:linear-gradient(90deg,${e.azulNavel},${e.azulMedio});margin-bottom:8px;border-radius:0 0 2px 2px"></div>`}function P(r="",i="",p=""){const s=i?`${a(z)} · ${a(i)}`:a(z),l=p||`${a(n.web)} &nbsp;|&nbsp; ${a(n.telefones)}`;return`
...
<div class="rpt-pagina-cliente" style="page-break-before:always">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:12px"><tr>
    <td width="48%" style="${o.sigCol}">
      ...
      ${v?`<div style="margin-top:6px"><img src="${v}" alt="Assinatura do técnico" width="200" height="55" style="max-width:200px;max-height:55px;border:1px solid ${e.cinzaBorda};border-radius:3px;display:block"></div>`:""}
    </td>
    <td width="4%"></td>
    <td width="48%" style="${o.sigCol}">
      <div style="${o.label}">Assinado pelo cliente</div>
      <div style="${o.sigName}">${a(s)}</div>
      ${h?`<div style="margin-top:6px"><img src="${h}" alt="Assinatura do cliente" width="200" height="55" style="max-width:200px;max-height:55px;border:1px solid ${e.cinzaBorda};border-radius:3px;display:block"></div>`:`<div style="height:55px;border-bottom:1.5px dashed ${e.cinzaBorda};margin-top:6px"></div>`}
    </td>
  </tr></table>`;if(f?.length>0){const g=f.sort((d,x)=>(d.data??"").localeCompare(x.data??"")).map((d,x)=>`<tr${x%2===1?` style="background:${e.cinza}"`:""}>
        <td style="${o.tdStyle};text-align:center;width:30px">${x+1}</td>
        <td style="${o.tdStyle}">${y(d.data)}</td>
        <td style="${o.tdStyle}">${a(w(d.periodicidade)||d.tipo||"—")}</td>
        <td style="${o.tdStyle};color:${e.muted}">${a(d.tecnico??"A designar")}</td>
      </tr>`).join("");b+=`
...
</div>`,b}function B(r=[],i="Documentação fotográfica"){if(!r.length)return"";const p=(l,c)=>`<div class="rpt-foto-item">
...
</section>`}export{n as E,e as P,t as T,T as a,P as b,C as c,B as d,E as e,S as h};
