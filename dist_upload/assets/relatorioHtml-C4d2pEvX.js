import{a as L,f as K}from"./datasAzores-vTPFpdRw.js";import{r as ba,a as R,g as ua,e as ga}from"./emailConfig-D1SCop7p.js";import{o as ma,q as va,k as $a,m as B,r as fa}from"./index-BPSrx_NR.js";import{c as ha,h as xa,a as ka,T as d,P as t,d as ya,e as wa,b as za}from"./relatorioBaseStyles-C-PEBt2j.js";function O(s,l){const a=String(s??"").trim();return/^#[0-9a-fA-F]{6}$/.test(a)?a.toLowerCase():/^#[0-9a-fA-F]{3}$/.test(a)?`#${a[1]}${a[1]}${a[2]}${a[2]}${a[3]}${a[3]}`.toLowerCase():l}function Sa(s,l){const a=O(s,"#000000"),g=Number.parseInt(a.slice(1,3),16),T=Number.parseInt(a.slice(3,5),16),D=Number.parseInt(a.slice(5,7),16);return`rgba(${g}, ${T}, ${D}, ${l})`}function Da(s,l,a,g,T=[],D={}){if(!s)return"";const h=ba(s,T),{subcategoriaNome:E,ultimoEnvio:z,logoUrl:V,istobalLogoUrl:Q,tecnicoObj:k,proximasManutencoes:_}=D,Y=V??"/manut/logo-navel.png",G=Q??"/manut/logo-istobal.png",e=ga,U=/istobal/i.test(String(a?.marca??""))||/istobal/i.test(String(E??"")),J=a?.marcaLogoUrl||(U?G:""),W=a?.marca?`Logotipo ${a.marca}`:"Logotipo da marca",F=O(a?.marcaCorHex,U?"#c8102e":"#1a4880"),X=Sa(F,.12),$=!!(s.tipoManutKaeser||ma(a?.marca)),x=s.tipoManutKaeser??"",S=x?va[x]:null,m=a?.posicaoKaeser??null,M=m!=null?fa(m):null,H=m!=null?B(m):null,j=M!=null?B(M):null,Z=s.dataCriacao?L(s.dataCriacao):"—",aa=s.dataAssinatura?L(s.dataAssinatura):"Pendente de assinatura",ea=l?.data?K(l.data,!0):"—",sa=s.dataAssinatura?K(s.dataAssinatura,!0):s.dataCriacao?K(s.dataCriacao,!0):"Pendente de preenchimento",ta=z?.data&&z?.destinatario?`Último envio por email: ${L(z.data)} para ${e(z.destinatario)}`:null,q=e(l?.tecnico||s?.tecnico||"—");s.assinaturaDigital&&R(s.assinaturaDigital),k?.assinaturaDigital&&R(k.assinaturaDigital);const oa=k?.telefone?e(k.telefone):"",P=Math.ceil((h??[]).length/2),la=(h??[]).slice(0,P),ra=(h??[]).slice(P),I=(p,i,r=0)=>{const n=s.checklistRespostas?.[p.id],y=n==="sim"?'<span class="badge-sim" style="background:#dcfce7;color:#14532d;padding:1.5px 6px;border-radius:8px;font-size:9px;font-weight:700;border:1px solid #86efac">SIM</span>':n==="nao"?'<span class="badge-nao" style="background:#fee2e2;color:#7f1d1d;padding:1.5px 6px;border-radius:8px;font-size:9px;font-weight:700;border:1px solid #fca5a5">NÃO</span>':'<span class="badge-nd" style="color:#374151">—</span>',b=(i+r)%2===1?`;background:${t.cinza}`:"";return`<tr style="border-bottom:1px solid ${t.bordaLeve}${b}"><td class="cl-num" style="width:1.6em;color:${t.muted};font-size:${d.label};padding:3px 4px;white-space:nowrap;vertical-align:top">${i+r+1}.</td><td class="cl-texto" style="padding:3px 6px 3px 4px;vertical-align:top;font-size:${d.pequeno};color:${t.texto}">${e(p.texto)}</td><td class="cl-badge" style="width:32px;text-align:center;padding:3px 2px;white-space:nowrap;vertical-align:top">${y}</td></tr>`};let c=`<!DOCTYPE html>
<html lang="pt">
<head>
<meta charset="utf-8">
...
${wa({tecnicoNome:q,tecnicoTelefone:oa,tecnicoAssinatura:k?.assinaturaDigital,clienteNome:s.nomeAssinante??"—",clienteAssinatura:s.assinaturaDigital,dataCriacao:Z,dataAssinatura:aa,declaracaoTexto:ua(l?.tipo==="montagem"?"montagem":"periodica"),proximasManutencoes:_})}
