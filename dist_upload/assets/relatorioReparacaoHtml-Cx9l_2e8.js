import{d as w,f as A}from"./index-y9UWHWV9.js";import{s as G,e as W,g as X}from"./emailConfig-mMFhqgpy.js";import{a as Z,T as r,P as s,c as q,h as aa,b as ta,d as ea,e as sa}from"./relatorioBaseStyles-Bd789_86.js";import"./vendor-Bk0NZ5Or.js";import"./vendor-pdf-hYLUQ4hX.js";import"./vendor-icons-BPQHcSY9.js";import"./vendor-datefns-COkYMph7.js";function k(a,l){const e=String(a??"").trim();return/^#[0-9a-fA-F]{6}$/.test(e)?e.toLowerCase():/^#[0-9a-fA-F]{3}$/.test(e)?`#${e[1]}${e[1]}${e[2]}${e[2]}${e[3]}${e[3]}`.toLowerCase():l}function N(a,l){const e=k(a,"#000000"),i=Number.parseInt(e.slice(1,3),16),b=Number.parseInt(e.slice(3,5),16),g=Number.parseInt(e.slice(5,7),16);return`rgba(${i}, ${b}, ${g}, ${l})`}function ba(a,l,e,i,b=[],g={}){if(!a)return"";const{subcategoriaNome:m,logoUrl:R,istobalLogoUrl:C,tecnicoObj:x}=g,S=R??"/manut/logo-navel.png",O=C??"/manut/logo-istobal.png",o=W,f=l?.origem==="istobal_email"||/istobal/i.test(String(e?.marca??""))||/istobal/i.test(String(m??"")),P=e?.marcaLogoUrl||(f?O:""),T=e?.marca?`Logotipo ${e.marca}`:"Logotipo da marca",c=k(e?.marcaCorHex,f?"#c8102e":"#b45309"),$=N(c,.09),D=N(c,.28),L=a.dataCriacao?w(a.dataCriacao,!0):"—",B=a.dataAssinatura?A(a.dataAssinatura):"Pendente de assinatura",v=a.dataRealizacao||l?.data||a.dataAssinatura||a.dataCriacao,I=v?w(v,!0):"Pendente de preenchimento",U=a.dataCriacao?A(a.dataCriacao):"—",H=e&&m?o(`${m} — ${e.marca} ${e.modelo} — Nº Série: ${e.numeroSerie}`):e?o(`${e.marca} ${e.modelo} — Nº Série: ${e.numeroSerie}`):"—";let n=[];try{const t=typeof a.pecasUsadas=="string"?JSON.parse(a.pecasUsadas||"[]"):a.pecasUsadas??[];n=Array.isArray(t)?t:[]}catch{}const E=n.length>0&&n.some(t=>t.descricao?.trim());let u={};try{const t=typeof a.checklistRespostas=="string"?JSON.parse(a.checklistRespostas||"{}"):a.checklistRespostas??{};u=t&&typeof t=="object"&&!Array.isArray(t)?t:{}}catch{}const h=Object.entries(u),K=h.length>0;let z=[];try{const t=typeof a.fotos=="string"?JSON.parse(a.fotos||"[]"):a.fotos??[];z=Array.isArray(t)?t:[]}catch{}const M=z.map(t=>G(t)).filter(Boolean),y={};b.forEach(t=>{y[t.id]=t.texto??t.descricao??t.nome??t.id});const j=([t,d],p)=>{const Q=y[t]??t,V=d==="OK"?'<span class="badge-sim" style="background:#dcfce7;color:#14532d;padding:1.5px 6px;border-radius:8px;font-size:9px;font-weight:700;border:1px solid #86efac">OK</span>':d==="NOK"?'<span class="badge-nao" style="background:#fee2e2;color:#7f1d1d;padding:1.5px 6px;border-radius:8px;font-size:9px;font-weight:700;border:1px solid #fca5a5">NOK</span>':`<span class="badge-nd" style="color:${s.muted}">N/A</span>`,Y=p%2===1?`;background:${s.cinza}`:"";return`<tr style="border-bottom:1px solid ${s.bordaLeve}${Y}"><td class="cl-num" style="width:1.6em;color:${s.muted};font-size:${r.label};padding:3px 4px;white-space:nowrap;vertical-align:top;text-align:right;padding-right:6px">${p+1}.</td><td class="cl-texto" style="padding:3px 6px 3px 4px;vertical-align:top;font-size:${r.pequeno};color:${s.texto}">${o(Q)}</td><td class="cl-badge" style="width:32px;text-align:center;padding:3px 2px;white-space:nowrap;vertical-align:top">${V}</td></tr>`},F=l?.origem==="istobal_email"?' <span class="rpt-istobal-badge">⚡ ISTOBAL</span>':"",J=Z("Relatório de Reparação","Nº Relatório",a.numeroRelatorio??"—",F),_=`
...
<section>
  ...
  <div class="rpt-grid">
    ...
    <div class="rpt-field"><span class="rpt-label">Equipamento</span><span class="rpt-value">${H}</span></div>
...
${ea({tecnicoNome:a.tecnico??"—",tecnicoTelefone:x?.telefone??"",tecnicoAssinatura:x?.assinaturaDigital??"",clienteNome:a.nomeAssinante??"—",clienteAssinatura:a.assinaturaDigital??"",dataCriacao:U,dataAssinatura:B,declaracaoTexto:X("reparacao"),proximasManutencoes:[]})}

${sa("",`Relatório de Reparação ${a.numeroRelatorio??""}`)}

</body>
</html>`}export{ba as relatorioReparacaoParaHtml};
